...
`store-regional-pricing` is a three-step pipeline for scaling subscription prices by country (purchasing power parity / PPP adjusted) and pushing them to App Store Connect and Google Play Console:

1. `cost-of-living.py` — fetches World Bank PPP conversion factors and resolves currency codes offline via `pycountry` + Babel's CLDR territory data → writes `cost_of_living_data.xlsx`
...
**Country code mapping**: `country_codes.json` maps World Bank country names → ISO alpha-2 (Google) and alpha-3 (Apple). `update_country_codes.py` regenerates this file from the World Bank API (alpha-2 + names) and `pycountry` (alpha-3, offline). restcountries.com's free v3.1 API was retired (301s to a deprecated-endpoint error body); its v5 replacement requires a signed-up API key, so both this script and `cost-of-living.py`'s currency-code lookup were moved off it entirely onto offline datasets (`pycountry` + Babel's CLDR territory data) rather than adding a new required credential.
